...
fn main() -> std::io::Result<()> {
    ...
    let unique_dates = sequence_counts.iter().filter(|&count| count.load(Ordering::Relaxed) > 0).count();
    ...
    let max_count = sequence_counts.iter().enumerate().map(|(index, count)| (1900_0000 + index, count.load(Ordering::Relaxed))).max_by_key(|&(_, count)| count).unwrap();
